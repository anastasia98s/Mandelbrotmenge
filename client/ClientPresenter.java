...
public class ClientPresenter {
    ...
    int stufenanzahl;
    double xMinimum, xMaximum, yMinimum, yMaximum; // Parameter des Ausschnitts
    double cr, ci;
    double zoomfaktor;
    ...
    public void init(ClientModel m, ClientView v) {
        ...
        try (FileInputStream config_input = new FileInputStream("client.config")) {
            ...
            xMinimum = Double.parseDouble(config_properties.getProperty("input.xMinimum"));
            xMaximum = Double.parseDouble(config_properties.getProperty("input.xMaximum"));
            yMinimum = Double.parseDouble(config_properties.getProperty("input.yMinimum"));
            yMaximum = Double.parseDouble(config_properties.getProperty("input.yMaximum"));
        } catch (Exception e) {
            v.updateInfo("Config not found!");
        }
    }
            ...
            for (int i = 0; i < stufenanzahl; i++) {
                if(!stopVideo){
                    if(mandel_result[i][0][0] != null){
                        ...
                        try {
                            Thread.sleep((int) zoomfaktor * 90);
                        } catch (InterruptedException e) {
                            v.updateInfo("Error: Videowiedergabe");
                        }
                    }else{
                        break;
                    }
